...
export class NftHoldingSyncWorker {
    ...
    async sync() {
        ...
        try {
            ...
            const { rows: stateRows } = await client.query(`SELECT last_synced_block FROM balance_sync_state
         WHERE chain_id=$1 AND sync_type='nft' FOR UPDATE`, [this.chainId]);
         ...
         ORDER BY block_number, log_index, batch_index`, [this.chainId, fromBlock.toString(), toBlock.toString()]);
         ...
         WHERE chain_id=$2 AND sync_type='nft'`, [toBlock.toString(), this.chainId]);
            await client.query('COMMIT');
            if (rows.length > 0)
                logger.debug({ from: fromBlock.toString(), to: toBlock.toString(), count: rows.length }, 'NFT 持有同步完成');
    ...
    async applyTransfer(client, row, blockNumber) {
        ...
        if (token_standard === 'ERC721') {
           ...
           DO UPDATE SET last_transfer_block=$5, updated_at=NOW()`, [this.chainId, contract_address, token_id, to_address, blockNumber.toString()]);
            ...
            // ERC1155: 减 from
            if (from_address !== ZERO_ADDRESS) {
                await client.query(`UPDATE nft_holdings
           SET amount=amount-$1, last_transfer_block=$2, updated_at=NOW()
           WHERE chain_id=$3 AND contract_address=$4
             AND token_id=$5 AND owner_address=$6`, [amountBn.toString(), blockNumber.toString(), this.chainId, contract_address, token_id, from_address]);
            }
            // 加 to
            if (to_address !== ZERO_ADDRESS) {
                await client.query(`INSERT INTO nft_holdings
