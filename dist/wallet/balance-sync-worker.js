...
export class BalanceSyncWorker {
    ...
    async sync() {
        ...
        try {
         ...
         WHERE chain_id=$1 AND sync_type='erc20' FOR UPDATE`, [this.chainId]);
         ...
         ON CONFLICT (chain_id, contract_address, holder_address) DO UPDATE
           SET balance_raw = GREATEST(0,
               ...
               updated_at = NOW()`, [this.chainId, fromBlock.toString(), toBlock.toString()]);
            await client.query(`UPDATE balance_sync_state
         SET last_synced_block=$1, updated_at=NOW()
         WHERE chain_id=$2 AND sync_type='erc20'`, [toBlock.toString(), this.chainId]);
            await client.query('COMMIT');
    ...
    async invalidateAffectedCache(fromBlock, toBlock) {
        try {
         ...
         WHERE chain_id=$1 AND status='indexed' AND block_number BETWEEN $2 AND $3
         LIMIT 500`, [this.chainId, fromBlock.toString(), toBlock.toString()]);
