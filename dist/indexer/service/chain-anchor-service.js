...
export class ChainAnchorService {
    ...
    blockReader;
    inFlight = new Map();
    constructor(pool, env, httpClient, blockAnchorRepo, writeSemaphore) {
        this.pool = pool;
        this.env = env;
    ...
    async ensureSegmented(chainId, fromBlock, toBlock) {
    ...
    async ensureRange(chainId, fromBlock, toBlock) {
        if (fromBlock > toBlock)
            return;
    ...
    async doEnsureRange(chainId, fromBlock, toBlock) {
        ...
        const headerMap = await prefetchBlockHeaders(this.blockReader, missingBlocks, this.env.ANCHOR_PREFETCH_CONCURRENCY);
    ...
    async collectMissingBlocks(chainId, fromBlock, toBlock) {
        const existing = await this.blockAnchorRepo.listExistingBlockNumbersInRange(chainId, fromBlock, toBlock);
        const missing = [];
    ...
    async writeAnchorsForBlocks(client, chainId, blockNumbers, headerMap) {
...
//# sourceMappingURL=chain-anchor-service.js.map