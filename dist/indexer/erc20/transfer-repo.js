...
export class Erc20TransferRepo {
    ...
    async upsertChunk(client, records) {
        ...
        records.forEach((r, idx) => {
            ...
            values.push(r.chainId, r.contractAddress.toLowerCase(), r.symbol, r.txHash, r.logIndex, r.blockNumber.toString(), r.blockTimestamp, r.fromAddress.toLowerCase(), r.toAddress.toLowerCase(), r.amountRaw, r.amount);
        });
        const result = await client.query(`INSERT INTO token_transfers
       ...
       ON CONFLICT (chain_id, tx_hash, log_index, block_number) DO NOTHING`, values);
        return result.rowCount ?? 0;
    }
    async markReorgedAfterBlock(client, chainId, contractAddress, afterBlock) {
        const result = await client.query(`UPDATE token_transfers SET status='reorged'
    ...
    async countInPartition(partitionName) {
        const { rows } = await this.pool.query(`SELECT COUNT(*)::bigint AS cnt FROM public.${partitionName} WHERE status='indexed'`);
