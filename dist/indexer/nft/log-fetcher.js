...
export class NftLogFetcher {
    ...
    async fetchLogs(contractAddress, tokenStandard, fromBlock, toBlock) {
        ...
        const [singles, batches] = await Promise.all([
            withRetry(() => this.client.getContractEvents({
                address: contractAddress,
                abi: erc1155Abi,
    ...
    async fetchWithAdaptiveRange(contractAddress, tokenStandard, fromBlock, toBlock, maxRange) {
        const allLogs = [];
        ...
        while (cursor <= toBlock) {
            ...
            try {
                ...
                const logs = await this.fetchWithAdaptiveRange(contractAddress, tokenStandard, cursor, mid, maxRange / 2n > 0n ? maxRange / 2n : 1n);
                allLogs.push(...logs);
                cursor = mid + 1n;
            }
        }
